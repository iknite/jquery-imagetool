...
(function($) {
    ...
    var handleMouseMove = function (mmevt) {
        var image = $(this);
    ...
    $.fn.extend({
        ...
        resize: function() {
            ...
            if (dim.width < dim.viewportWidth) {
                dim.height = parseInt(dim.imageHeight * (dim.viewportWidth / dim.imageWidth), 10);
                dim.width = dim.viewportWidth;
                wasResized = false;
            }

            if (dim.height < dim.viewportHeight) {
                dim.width = parseInt(dim.imageWidth * (dim.viewportHeight / dim.imageHeight), 10);
                dim.height = dim.viewportHeight;
                wasResized = false;
            }

            if (dim.width > dim.imageMaxWidth) {
                dim.height = parseInt(dim.height * (dim.imageMaxWidth / dim.width), 10);
